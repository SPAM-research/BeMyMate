...
def last_message_is_suggestion(problem):
    ...
    last_message = problem.chat[-1]["message"]
    return re.search(
        "Puedes.+definir|¿te refieres a |Te sugiero definir una letra para denotar la cantidad",
        last_message,
    )
...
def get_message_for_suggestion(problem):
    ...
    if match := re.search(
        "Puedes intentar definir la ecuación.+?(?P<sug>.+=.+)", last_message
    ):
        # DONE
        ...
        return (
            f"{usable_vars.pop()} es {match.group('var').strip()}"
            if match
            else "FAIL IN HANDLING SECOND SUGGESTION"
        )
    elif match := re.search(
        "Puedes intentar definir (?P<var>.+) en función de", last_message
    ):
        # DONE
        # print("HANDLING THIRD SUGGESTION")
        return (
            f"{usable_vars.pop()} es {match.group('var')}"
            if match
        ...
        "¿te refieres a .*¿quieres que denotemos.*", last_message, flags=re.DOTALL
    ):
        # DONE
