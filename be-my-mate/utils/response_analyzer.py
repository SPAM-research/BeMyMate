...
def definitions_validator(
    definitions: list[str], outputs: list[str], notebook: list[str]
) -> list[str]:
    ...
    new_filtered_definitions = [
        definition
        for definition in new_definitions
        if pattern.match(definition)
        and definition.split(" is ", 1)[1].upper() not in notebook_descriptions
    ]
...
def equations_validator(
    equations: list[str],
    outputs: list[str],
    notebook: list[str],
    notebook_equations: list[str],
) -> list:
    ...
    for equation in new_equations:
        ...
        if (
            letters_in_equation.issubset(notebook_letters)
            and equation not in notebook_equations
            and "?" not in equation
        ):
    ...
    for equation in new_filtered_equations:
        try:
            if (
                "=" in equation
            ):  # if an = is present, analyze each side as its own equation
